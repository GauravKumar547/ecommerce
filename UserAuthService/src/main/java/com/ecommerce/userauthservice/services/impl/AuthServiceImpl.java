...
import com.ecommerce.userauthservice.clients.KafkaProducerClient;
import com.ecommerce.userauthservice.dtos.EmailDto;
import com.ecommerce.userauthservice.models.Session;
import com.ecommerce.userauthservice.models.User;
import com.ecommerce.userauthservice.repos.SessionRepository;
...
//            kafkaProducerClient.sendMessage("user-signup-topic", objectMapper.writeValueAsString(emailDto));
    ...
    @Override
    public Pair<Long, String> refreshToken(String token) {
        try{
            JwtParser parser = Jwts.parser().verifyWith(secretKey).build();
            Claims claims = parser.parseSignedClaims(token).getPayload();

            Long uid = claims.get("user_id", Long.class);
            Optional<Session> sessionOptional = sessionRepository.findByTokenAndUserId(token, uid);
            if(sessionOptional.isEmpty()){
            ...
            payload.put("iat", currentMillis);
            payload.put("exp", currentMillis + 3600000);
            String newToken = Jwts.builder().claims(payload).signWith(secretKey).compact();
            session.setToken(newToken);
            sessionRepository.save(session);
            return new Pair<>(uid,newToken);
        }catch (UserUnauthorizedException e){
            throw e;
        }
        catch(Exception e){
